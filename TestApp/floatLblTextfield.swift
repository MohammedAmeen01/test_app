...
@IBDesignable class FloatLabelTextField: UITextField {
    ...
    @IBInspectable var titleTextColour:UIColor = UIColor.blue {
        didSet {
            if !isFirstResponder {
            if title.text?.range(of: "*") != nil
            {
                
                let name: String = title.text!
                let endIndex = name.index(name.endIndex, offsetBy: -2)
                let truncated = name.substring(to: endIndex)
                let attrs1 = [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 10.5), NSAttributedString.Key.foregroundColor : titleTextColour]
                let attributedString1 = NSMutableAttributedString(string:"*", attributes:attrs1)
                let attrs2 = [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 10.5), NSAttributedString.Key.foregroundColor : titleTextColour]
                
                let attributedString2 = NSMutableAttributedString(string:truncated, attributes:attrs2)
    ...
    // MARK:- Add atributes to label
    
    func addAtributedText() -> Void {
        if title.text?.range(of: "*") != nil
        {
            let name: String = title.text!
    ...
    //Helper.UIColorFromRGB(rgbValue: 0x999999)
    
    @IBInspectable var titleActiveTextColour:UIColor = UIColor.blue {
        didSet {
            if isFirstResponder {
                if title.text?.range(of: "*") != nil
                {
...
//            title.textColor = titleTextColour
    ...
    override func textRect(forBounds bounds:CGRect) -> CGRect {
        ...
        if let txt = text , !txt.isEmpty {
            ...
            r = r.inset(by: UIEdgeInsets(top: top, left: 0.0, bottom: 0.0, right: 0.0)) //UIEdgeInsetsInsetRect(r, UIEdgeInsets(top: top, left: 0.0, bottom: 0.0, right: 0.0))
        }
        return r.integral
    }
